...
### ALDO e Check serale: da Slack a email Gmail (17/09/2026)

Piano `plans/2026-09-17-aldo-checkserale-email-gmail.md`. Alberto ha segnalato di non riuscire a tenere traccia dei DM Slack ("non riesco a tenere traccia ed avere un alert per ricordare a che punto sono") e ha chiesto un canale diverso, con Slack spento del tutto per questi due agenti (non tenuto come backup). Valutate Telegram/WhatsApp (richiedono un bot/connettore che non esiste in questo workspace) e la notifica push nativa (funziona solo in sessione interattiva con Remote Control collegato, inutile per una routine cloud non presidiata): scelta l'email, perché Gmail è già connesso e già usato da ALDO stesso.

**Cosa è cambiato:** entrambi i prompt ora inviano con `send_message` (Gmail, invio diretto, non bozza) invece di `slack_send_message`. Oggetto fisso e riconoscibile: `ALDO | Agenda [GG/MM]` e `Check serale | [GG/MM]`. Il meccanismo con cui ALDO trova le chiusure di task di Alberto (prima `slack_read_thread` sul DM) ora usa `search_threads`/`get_thread` (Gmail) cercando per prefisso di oggetto. Cron, orari e modello (Sonnet 5) invariati.

**Etichette Gmail non applicate:** il piano prevedeva due etichette (`ALDO`, `Check-serale`) per rinforzare il riconoscimento oltre all'oggetto. Il connettore Gmail collegato non ha lo scope `gmail.labels`/`gmail.modify` (solo lettura e invio) — verificato il 17/09/2026 con un tentativo diretto di `create_label`, fallito con "Insufficient scope". I prompt sono stati scritti per non tentare `label_message`/`create_label` (fallirebbe a ogni run). Se in futuro Alberto autorizza uno scope Gmail più ampio dalle impostazioni connettori di claude.ai, si possono aggiungere.

**Verificato lo stesso giorno:** `send_message` funziona con lo scope attuale (email di test inviata e ricevuta). Run manuale di Check serale lanciato con `RemoteTrigger run` per confermare il comportamento end-to-end prima del prossimo giro naturale.

**Rischio noto, non risolto da questo cambio:** la casella Gmail di Alberto ha 1.872 email non lette al 17/09/2026. Un'email con oggetto fisso aiuta a riconoscerla, ma non risolve da sola il problema se la casella resta così piena — a differenza di un'etichetta filtrabile (bloccata dallo scope, vedi sopra). Da tenere d'occhio nelle prime settimane.

**PIERO, VERA, OTTO, MARCO restano su Slack**, invariati: decisione esplicita di Alberto in questa sessione, rivalutabile in futuro con un piano a sé.
...
- **Slack DM a se stesso** = canale di output standard per PIERO, VERA, OTTO, MARCO (auto-approvato). ALDO e Check serale sono passati a email Gmail inviata il 17/09/2026 (Alberto non riusciva a tenere traccia dei DM Slack) — vedi sezione dedicata sopra. Qualsiasi messaggio a terzi resta manuale.
